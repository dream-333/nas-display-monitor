...
async function loadConfiguration() {
  ...
  for (const [id, key] of [['display-ip', 'display_ip'], ['port', 'port'], ['interval', 'interval'], ['transport', 'transport']]) $(id).value = configuration[key];
  $('enabled').checked = configuration.enabled;
  modeFields(); await refreshDevices();
}
function readForm() { return {display_ip: $('display-ip').value.trim(), port: Number($('port').value), interface: $('interface').value, interval: Number($('interval').value), gpu_device: $('gpu-device').value, disks: [$('disk1').value, $('disk2').value], enabled: $('enabled').checked, transport: $('transport').value, usb_port: $('usb-port').value || 'auto', sys_sensor: $('sys-sensor').value, cpu_sensor: $('cpu-sensor').value}; }
...
function updateStatus(data) {
  ...
  $('target').textContent = data.transport === 'usb' ? 'USB-C 直连' : data.display_ip || '尚未设置屏幕地址';
...
$('logout').addEventListener('click', async () => { try { await api('logout', {}); configuration = null; showLogin(); } catch(e) { toast(e.message); } });
